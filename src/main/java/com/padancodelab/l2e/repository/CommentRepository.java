...
import com.padancodelab.l2e.domain.Comment;
...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query(
        value = "select distinct comment from Comment comment left join fetch comment.learningPath",
    ...
    @Query("select distinct comment from Comment comment left join fetch comment.learningPath")
    List<Comment> findAllWithToOneRelationships();

    @Query("select comment from Comment comment left join fetch comment.learningPath where comment.id =:id")
